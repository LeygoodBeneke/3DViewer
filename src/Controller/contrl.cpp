...
public:
    ...
    void SetBackgroundColor(QColor color) {_global_settings.SetBackgroundColor(color);};
    
    void SetEdgesType(int type) {_global_settings.SetEdgesType(type);};
    void SetVerticesDisplayMethod(int type) {_global_settings.SetVerticesDisplayMethod(type);};
...
private:
    s21::GlobalViewSettings& _global_settings = s21::GlobalViewSettings::Instance();
};
